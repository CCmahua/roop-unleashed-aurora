    ...
    with gr.Tab("🎭 人脸替换"):
        with gr.Row(variant='panel'):
            with gr.Column(scale=2):
                with gr.Row():
                    input_faces = gr.Gallery(label="输入人脸图库", allow_preview=False, preview=False, height=138, columns=64, object_fit="scale-down", interactive=False)
                    target_faces = gr.Gallery(label="目标人脸图库", allow_preview=False, preview=False, height=138, columns=64, object_fit="scale-down", interactive=False)
                with gr.Row():
                    bt_move_left_input = gr.Button("⬅ 向左移动", size='sm')
                    bt_move_right_input = gr.Button("➡ 向右移动", size='sm')
                    bt_move_left_target = gr.Button("⬅ 向左移动", size='sm')
                    bt_move_right_target = gr.Button("➡ 向右移动", size='sm')
                with gr.Row():
                    bt_remove_selected_input_face = gr.Button("❌ 移除选中", size='sm')
                    bt_clear_input_faces = gr.Button("💥 清除全部", variant='stop', size='sm')
                    bt_remove_selected_target_face = gr.Button("❌ 移除选中", size='sm')
                    bt_add_local = gr.Button('从本地添加文件', size='sm')

                with gr.Row():
                    with gr.Column(scale=2):
                        with gr.Accordion(label="高级遮罩", open=False):
                            ...
                            bt_toggle_masking = gr.Button(
                                "切换手动遮罩", variant="secondary", size="sm"
                            )
                            ...
                            clip_text = gr.Textbox(
                                label="要遮罩并在假脸上恢复的对象列表",
                                value="cup,hands,hair,banana",
                                interactive=False,
                            )
                            bt_preview_mask = gr.Button(
                                "👥 显示遮罩预览", variant="secondary"
                            )
                    with gr.Column(scale=2):
                        local_folder = gr.Textbox(show_label=False, placeholder="/content/", interactive=True)
                with gr.Row(variant='panel'):
                    bt_srcfiles = gr.Files(label='源图像或人脸集', file_count="multiple", file_types=["image", ".fsz"], elem_id='filelist', height=233)
                    bt_destfiles = gr.Files(label='目标文件', file_count="multiple", file_types=["image", "video"], elem_id='filelist', height=233)
                with gr.Row(variant='panel'):
                    gr.Markdown('')
                    forced_fps = gr.Slider(minimum=0, maximum=120, value=0, label="视频帧率", info='如果不为0则覆盖检测到的帧率', step=1.0, interactive=True, container=True)

            with gr.Column(scale=2):
                previewimage = gr.Image(label="预览图像", height=576, interactive=False, visible=True, format=get_gradio_output_format())
                maskimage = gr.ImageEditor(label="手动遮罩图像", sources=["clipboard"], transforms="", type="numpy",
                                             brush=gr.Brush(color_mode="fixed", colors=["rgba(255, 255, 255, 1"]), interactive=True, visible=False)
                with gr.Row(variant='panel'):
                    fake_preview = gr.Checkbox(label="人脸替换帧", value=False)
                    bt_refresh_preview = gr.Button("🔄 刷新", variant='secondary', size='sm')
                    bt_use_face_from_preview = gr.Button("使用此帧的人脸", variant='primary', size='sm')
                with gr.Row():
                    preview_frame_num = gr.Slider(1, 1, value=1, label="帧号", info='0:00:00', step=1.0, interactive=True)
                with gr.Row():
                    text_frame_clip = gr.Markdown('处理帧范围 [0 - 0]')
                    set_frame_start = gr.Button("⬅ 设为起始", size='sm')
                    set_frame_end = gr.Button("➡ 设为结束", size='sm')
        with gr.Row(visible=False) as dynamic_face_selection:
            with gr.Column(scale=2):
                face_selection = gr.Gallery(label="检测到的人脸", allow_preview=False, preview=False, height=138, object_fit="cover", columns=32)
            with gr.Column():
                bt_faceselect = gr.Button("☑ 使用选中的人脸", size='sm')
                bt_cancelfaceselect = gr.Button("完成", size='sm')
            with gr.Column():
                gr.Markdown(' ') 

        with gr.Row(variant='panel'):
            with gr.Column(scale=1):
                selected_face_detection = gr.Dropdown(swap_choices, value="First found", label="指定人脸替换选择")
            with gr.Column(scale=1):
                num_swap_steps = gr.Slider(1, 5, value=1, step=1.0, label="替换步骤数", info="更多步骤可能增加相似度")
            with gr.Column(scale=2):
                ui.globals.ui_selected_enhancer = gr.Dropdown(["None", "Codeformer", "DMDNet", "GFPGAN", "GPEN", "Restoreformer++"], value="None", label="选择后处理")

        with gr.Row(variant='panel'):
            with gr.Column(scale=1):
                max_face_distance = gr.Slider(0.01, 1.0, value=0.65, label="最大人脸相似度阈值", info="0.0 = 完全相同 1.0 = 无相似度")
            with gr.Column(scale=1):
                ui.globals.ui_upscale = gr.Dropdown(["128px", "256px", "512px"], value="128px", label="上采样到", interactive=True)
            with gr.Column(scale=2):
                ui.globals.ui_blend_ratio = gr.Slider(0.0, 1.0, value=0.65, label="原始/增强图像混合比例", info="仅用于激活后处理时")

        with gr.Row(variant='panel'):
            with gr.Column(scale=1):
                video_swapping_method = gr.Dropdown(["Extract Frames to media","In-Memory processing"], value="In-Memory processing", label="选择视频处理方法", interactive=True)
                no_face_action = gr.Dropdown(choices=no_face_choices, value=no_face_choices[0], label="未检测到人脸时的操作", interactive=True)
                vr_mode = gr.Checkbox(label="VR模式", value=False)
            with gr.Column(scale=1):
                with gr.Group():
                    autorotate = gr.Checkbox(label="自动旋转水平人脸", value=True)
                    roop.globals.skip_audio = gr.Checkbox(label="跳过音频", value=False)
                    roop.globals.keep_frames = gr.Checkbox(label="保留帧(仅在提取帧时相关)", value=False)
                    roop.globals.wait_after_extraction = gr.Checkbox(label="创建视频前等待用户按键", value=False)
        ...
        with gr.Row(variant='panel'):
            ...
            with gr.Column():
                ...
                gr.Button("👀 打开输出文件夹", size='sm').click(fn=lambda: util.open_folder(roop.globals.output_path))
            with gr.Column(scale=2):
                output_method = gr.Dropdown(["File","Virtual Camera", "Both"], value="File", label="选择输出方式", interactive=True)
        with gr.Row(variant='panel'):
            with gr.Column():
                resultfiles = gr.Files(label='处理后的文件', interactive=False)
            with gr.Column():
                resultimage = gr.Image(type='filepath', label='最终图片', interactive=False )
                resultvideo = gr.Video(label='最终视频', interactive=False, visible=False)
