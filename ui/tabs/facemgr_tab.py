    ...
    with gr.Tab("👨‍👩‍👧‍👦 人脸管理"):
        with gr.Row():
            gr.Markdown("""
                        ...
                        """)
        with gr.Row():
            videoimagefst = gr.Image(label="从视频帧中截取人脸", height=576, interactive=False, visible=True, format="jpeg")
        with gr.Row():
            frame_num_fst = gr.Slider(1, 1, value=1, label="帧数", info='0:00:00', step=1.0, interactive=False)
            fb_cutfromframe = gr.Button("使用此帧中的人脸", variant='secondary', interactive=False)
        with gr.Row():
            fb_facesetfile = gr.Files(label='人脸集', file_count='single', file_types=['.fsz'], interactive=True)
            fb_files = gr.Files(label='输入文件', file_count="multiple", file_types=["image", "video"], interactive=True)
        with gr.Row():
            with gr.Column():
            ...
            with gr.Column():
                gr.Markdown(' ')
        with gr.Row():
            faces = gr.Gallery(label="此人脸集中的人脸", allow_preview=True, preview=True, height=128, object_fit="scale-down")
        with gr.Row():
            fb_remove = gr.Button("删除选中项", variant='secondary')
            fb_update = gr.Button("创建/更新人脸集文件", variant='primary')
            fb_clear = gr.Button("清除全部", variant='stop')
