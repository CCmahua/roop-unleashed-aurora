...
def extras_tab():
    ...
    with gr.Tab("🎉 附加功能"):
        with gr.Row():
            files_to_process = gr.Files(label='要处理的文件', file_count="multiple", file_types=["image", "video"])
        with gr.Row(variant='panel'):
            with gr.Accordion(label="视频/GIF", open=False):
                with gr.Row(variant='panel'):
                    with gr.Column():
                        gr.Markdown("""
                                    # 简易视频编辑器
    ...
    """)
                    with gr.Column():
                        cut_start_time = gr.Slider(0, 1000000, value=0, label="起始帧", step=1.0, interactive=True)
                    with gr.Column():
                        cut_end_time = gr.Slider(1, 1000000, value=1, label="结束帧", step=1.0, interactive=True)
                    with gr.Column():
                        extras_chk_encode = gr.Checkbox(label='重新编码视频(对于不同编码的视频是必需的)', value=False)
                        start_cut_video = gr.Button("剪切视频")
                        start_extract_frames = gr.Button("提取帧")
                        start_join_videos = gr.Button("合并视频")
    ...
    """)
                    with gr.Column():
                        extras_fps = gr.Slider(minimum=0, maximum=120, value=30, label="视频帧率", step=1.0, interactive=True)
                        extras_images_folder = gr.Textbox(show_label=False, placeholder="/content/", interactive=True)
                    with gr.Column():
                        extras_chk_creategif = gr.Checkbox(label='从视频创建GIF', value=False)
                        extras_create_video=gr.Button("创建")
                with gr.Row(variant='panel'):
                    with gr.Column():
                        gr.Markdown("""
                                    # 从GIF创建视频
    """)
                    with gr.Column():
                        extras_video_fps = gr.Slider(minimum=0, maximum=120, value=0, label="视频帧率", step=1.0, interactive=True)
                    with gr.Column():
                        extras_create_video_from_gif=gr.Button("创建")

        with gr.Row(variant='panel'):
            with gr.Accordion(label="全帧处理", open=True):
                with gr.Row(variant='panel'):
                    filterselection = gr.Dropdown(filternames, value="None", label="着色器/滤镜效果", interactive=True)
                    upscalerselection = gr.Dropdown(upscalernames, value="None", label="增强器", interactive=True)
                with gr.Row(variant='panel'):
                    start_frame_process=gr.Button("开始处理")

        with gr.Row():
            gr.Button("👀 打开输出文件夹", size='sm').click(fn=lambda: util.open_folder(roop.globals.output_path))
        with gr.Row():
            extra_files_output = gr.Files(label='输出文件', file_count="multiple")
